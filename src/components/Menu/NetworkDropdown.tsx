...
import { FlowMainnetNetworkInfo, BASE_SEPOLIANetworkInfo, SUPPORTED_NETWORK_VERSIONS } from 'constants/networks'
          ...
          <AutoColumn $gap="16px">
            ...
            {SUPPORTED_NETWORK_VERSIONS.map((n) => {
              return (
                <StyledInternalLink
                  key={n.id}
                  to={`${n === FlowMainnetNetworkInfo || n === BASE_SEPOLIANetworkInfo ? '' : '/' + n.route}/`}
                >
                  <NetworkRow
                    onClick={() => {
                      setShowMenu(false)
