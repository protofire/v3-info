...
import { getPoolHideList } from '../../constants/index'
...
export default function PoolTable({ poolDatas, maxItems = MAX_ITEMS }: { poolDatas: PoolData[]; maxItems?: number }) {
  ...
  const sortedPools = useMemo(() => {
    return poolDatas
      ? poolDatas
          .filter((x) => !!x && !getPoolHideList(currentNetwork.chainId).includes(x.address))
          .sort((a, b) => {
            if (a && b) {
      ...
      : []
  }, [currentNetwork.chainId, maxItems, page, poolDatas, sortDirection, sortField])
